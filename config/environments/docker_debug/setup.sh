...
source config/environments/docker_debug/config.sh

# Start Docker containers
docker run -d -it --cap-add=SYS_PTRACE --security-opt seccomp=unconfined -v $(pwd)/config:/config -p 1234:1234 --name channel $DOCKER_IMAGE_FLOWEMU
